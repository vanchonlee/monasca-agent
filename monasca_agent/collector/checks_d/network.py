    ...
    def check(self, instance):
        ...
        for nic_name in nic_stats.keys():
            if self._is_nic_monitored(nic_name, excluded_ifaces, exclude_iface_re):
                ...
                if nic.isup:
                    self.gauge('net.int_status', 0, device_name=nic_name, dimensions=dimensions, delegated_tenant=self.delegated_tenant_id)
                else:
                    self.gauge('net.int_status', 1, device_name=nic_name, dimensions=dimensions, delegated_tenant=self.delegated_tenant_id)
