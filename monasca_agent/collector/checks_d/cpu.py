    ...
    def check(self, instance):
        ...
        for key, value in data.items():
            if data[key] is None or instance.get('cpu_idle_only') and 'idle_perc' not in key:
                continue
            self.gauge(key, value, dimensions,delegated_tenant=self.delegated_tenant_id)
            num_of_metrics += 1

        if send_rollup_stats:
            self.gauge('cpu.total_logical_cores', psutil.cpu_count(logical=True), dimensions, delegated_tenant=self.delegated_tenant_id)
            num_of_metrics += 1
