    ...
    def _collect_metrics(self, host, db, dimensions, options):
        ...
        if 'Innodb_page_size' in results:
            ...
            self.gauge("mysql.innodb.buffer_pool_free",
                       innodb_buffer_pool_pages_free, dimensions=dimensions, delegated_tenant=self.delegated_tenant_id)
            self.gauge("mysql.innodb.buffer_pool_used",
                       innodb_buffer_pool_pages_used, dimensions=dimensions, delegated_tenant=self.delegated_tenant_id)
            self.gauge("mysql.innodb.buffer_pool_total",
                       innodb_buffer_pool_pages_total, dimensions=dimensions, delegated_tenant=self.delegated_tenant_id)

        if 'galera_cluster' in options and options['galera_cluster']:
            value = self._collect_scalar('wsrep_cluster_size', results)
            self.gauge('mysql.galera.wsrep_cluster_size', value, dimensions=dimensions, delegated_tenant=self.delegated_tenant_id)
        ...
        if 'replication' in options and options['replication']:
            ...
            if slave_running is not None:
                if slave_running.lower().strip() == 'on':
                    ...
                    slave_running = 0
                self.gauge("mysql.replication.slave_running", slave_running, dimensions=dimensions, delegated_tenant=self.delegated_tenant_id)
    ...
    def _rate_or_gauge_statuses(self, statuses, dbResults, dimensions):
        for status, metric in statuses.items():
            ...
            if value is not None:
                if metric_type == RATE:
                    self.rate(metric_name, value, dimensions=dimensions, delegated_tenant=self.delegated_tenant_id)
                elif metric_type == GAUGE:
                    self.gauge(metric_name, value, dimensions=dimensions, delegated_tenant=self.delegated_tenant_id)
                    ...
                    try:
                        ...
                        if result[col_idx] is not None:
                            if metric_type == GAUGE:
                                self.gauge(metric, float(result[col_idx]), dimensions=dimensions, delegated_tenant=self.delegated_tenant_id)
                            elif metric_type == RATE:
                                self.rate(metric, float(result[col_idx]), dimensions=dimensions, delegated_tenant=self.delegated_tenant_id)
                            else:
                                self.gauge(metric, float(result[col_idx]), dimensions=dimensions, delegated_tenant=self.delegated_tenant_id)
                        else:
